...
COVER_PLATFORMS = ("bilibili", "toutiao", "wechat", "yidian", "zhihu")
...
def _is_landscape_cover_image(candidate: Path) -> bool:
    try:
        with Image.open(candidate) as image:
            width, height = image.size
    except Exception:
...
def assign_covers(
    ...
    *,
    ...
    landscape_files = [p for p in pool_files if _is_landscape_cover_image(p)]
    if landscape_files:
        pool_files = landscape_files
    ...
    for article_id in articles_sorted:
        ...
        prefer = [p for p in candidates if p not in used_globally]
