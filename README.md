...
**Ordo-Publish** 是本地编排的多平台发布引擎。Markdown 是单一内容源；微信公众号必须通过 VPS 固定公网 IP 调用官方 API，知乎、头条号、简书、一点号、B 站专栏使用本机仓库隔离浏览器。本机不得直接调用微信 API。
...
- 微信任务始终经专用 SSH/SCP adapter 交给 VPS worker；配置缺失时阻断，禁止回退本机。
- `--remote vps` 只控制“整批任务远端托管”应急模式，不改变微信本来就必须走 VPS 的规则。
...
- `wechat_publisher.py`：仅供 VPS worker 使用的微信公众号 API 发布器
...
微信使用 VPS worker 调用 API，不需要浏览器登录。本机直接运行 `wechat_publisher.py` 会被拒绝。
...
1. 微信 VPS API `draft` 组，不启动浏览器；本机只负责打包和 SSH/SCP。
2. 全部待处理浏览器平台组成一个 `publish` 组，共享一个隔离 browser context。

每组命令生成独立 `run_id`，只读取该 `run_id` 对应的 `publish_records.csv` 行，防止旧记录或并发记录冒充本轮结果。`.ordo/auto_publish_state.json` 和 `publish_records.csv` 共同用于幂等判断；`[INFO] 没有未处理文章` 表示安全 no-op。
...
整批 VPS 托管仍保留为应急能力，不由自动 monitor 触发。它不同于微信专用 VPS API 路线；只有整批远端运行浏览器平台时才需要人工确认远端版本、浏览器/CDP 和登录态：
